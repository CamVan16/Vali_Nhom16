...
const ProductInfo = ({ id, name, price, discount, img, sizes, description, onImageClick, onSizeClick }) => {
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      if (!itemExists) {
        incrementItemCount(); // Chỉ tăng khi mục mới được thêm vào
      }
      ...
      <div style={{ marginTop: '10px' }}>
        ...
        {sizes.map((size) => (
          <button
            ...
            onClick={() => { setSelectedSize(size); onSizeClick(size); }}
          >
            {size}
          </button>
        ))}
